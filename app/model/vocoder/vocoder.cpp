...
void vocoder::process(const dsp_input& in, dsp_output& out)
{
    ...
    else
    {
        ...
        float *filter = this->channel_fft.data() + this->window_size / 2;

        arm_rfft_fast_f32(&this->fft_conv, cenv_in, cenv_out, 0);
        std::swap(cenv_in, cenv_out);
        arm_cmplx_mult_cmplx_f32(cenv_in + 2, filter + 2, cenv_out + 2, this->window_size / 4 - 1);
        cenv_out[0] = cenv_in[0] * filter[0]; // DC
        cenv_out[1] = cenv_in[1] * filter[1]; // Nyquist
        std::swap(cenv_in, cenv_out);
        ...
        std::swap(menv_in, menv_out);
        arm_cmplx_mult_cmplx_f32(menv_in + 2, filter + 2, menv_out + 2, this->window_size / 4 - 1);
        menv_out[0] = menv_in[0] * filter[0]; // DC
        menv_out[1] = menv_in[1] * filter[1]; // Nyquist
        std::swap(menv_in, menv_out);
