...
echo::echo(float blur, float time, float feedback, mode_type mode) : effect { effect_id::echo, "echo" },
...
void echo::process(const dsp_input_t& in, dsp_output_t& out)
{
    ...
    [this](auto input)
    {
        ...
        if (this->blur > 0) this->iir_lp.process(&in_d, &in_df, 1);
...
void echo::set_blur(float blur)
{
    ...
    if (this->blur == blur || mode == mode_type::delay)
        return;
...
void echo::set_feedback(float feedback)
{
    ...
    else if (this->mode == mode_type::echo)
    {
