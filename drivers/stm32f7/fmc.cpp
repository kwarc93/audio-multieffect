...
void fmc::sdram::send_cmd(fmc::sdram::bank bank, fmc::sdram::cmd cmd, uint32_t param)
{
    ...
    case fmc::sdram::cmd::auto_refresh:
        FMC_Bank5_6->SDCMR |= (static_cast<uint32_t>(param)) << FMC_SDCMR_NRFS_Pos;
        break;
    case fmc::sdram::cmd::load_mode_register:
        FMC_Bank5_6->SDCMR |= (static_cast<uint32_t>(param)) << FMC_SDCMR_MRD_Pos;
        break;
    default:
        break;
    }

    FMC_Bank5_6->SDCMR |= (static_cast<uint32_t>(bank)) << FMC_SDCMR_CTB2_Pos;
    FMC_Bank5_6->SDCMR |= (static_cast<uint32_t>(cmd)) << FMC_SDCMR_MODE_Pos;
}
